...
public class XLSread {
	...
	public static void main(String[] args) {
		...
		String filePath ="C://Users/test/Downloads/Blazz Pty Ltd - MYOB AE - FA Schedule (2023).xls";
			...
			for (Row row : sheet) {
				if (row.getRowNum() > 10) {
					...
					if(empCellCount > 2 || flag) {
						Cell checkCell = sheet.getRow(row.getRowNum()).getCell(0);
						if(checkCell != null && !checkCell.getStringCellValue().equalsIgnoreCase("grand total") && checkCell.getStringCellValue().matches(regex)) {
							headerRowNum = row.getRowNum()+1;
							flag = false;
							continue;
						}
						if(checkCell != null && checkCell.getStringCellValue().equalsIgnoreCase("grand total")) {
							flag = false;
							break;
						}
